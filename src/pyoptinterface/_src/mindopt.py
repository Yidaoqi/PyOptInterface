...
from .mindopt_model_ext import RawModel, Env, load_library
...
def detected_libraries():
    ...
    # Environment
    home = os.environ.get("MINDOPT_HOME", None)
...
def autoload_library():
    ...
    for lib in libs:
        ...
        if ret:
            logging.info(f"Loaded Mindopt library: {lib}")
            return True
    return False
...
def get_primalstatus(model):
    ...
    if termination_status in (
        ...
        has_solution = model.get_raw_attribute_int("HasSolution")
        if has_solution != 0:
...
model_attribute_get_func_map = {
    ...
    ModelAttribute.SolverName: lambda _: "Mindopt",
...
def get_constraint_name(model, constraint):
    ...
    attr_name_dict = {
        ...
        ConstraintType.Quadratic: "QCName",
    }
...
class Model(RawModel):
    ...
    def get_raw_parameter(self, param_name: str):
        param_type = mindopt_raw_type_map[self.raw_parameter_type(param_name)]
        get_function_map = {
    ...
    def get_model_raw_attribute(self, name: str):
        param_type = mindopt_raw_type_map[self.raw_attribute_type(name)]
        get_function_map = {
